...
def network_pf(network, snapshots=None, skip_pre=False, x_tol=1e-6, x_tol_outer=1e-3, use_seed=False,
               distribute_slack=False, slack_weights='p_set', inverter_control=False, oltc_control=False):
    """
    ...
    oltc_control : bool, default False
        If ``True``, activates outerloop which considers on load tap changer (oltc) transformer control on those
        transformers which their "oltc" attribute is activated (True).
    ...
    return _network_prepare_and_run_pf(network, snapshots, skip_pre, linear=False, x_tol_outer=x_tol_outer, x_tol=x_tol,
                                       ...
                                       slack_weights=slack_weights, inverter_control=inverter_control, oltc_control=oltc_control)
