...
'''
This example is for validation of active power curtailment control strategy
P(V) by plotting the controller output and its droop characteristic in the
same figure.
'''
...
n.generators.control_strategy = 'p_v'
power_flow()
...
Results_p = n.generators_t.p.loc[:, 'My Gen 1':'My Gen 29'] / \
        n.generators_t.p_set.loc[:, 'My Gen 1':'My Gen 29'] * 100

# controller droop characteristic method simplified
def p_v(v_pu_bus):
    v_pu_cr = n.generators.loc['My Gen 26', 'v_pu_cr']
    v_max_curtail = n.generators.loc['My Gen 26', 'v_max_curtail']
...
plt.plot(Results_droop['v_mag'], Results_droop["p/pmaxdroop"],
         label="P(U) droop characteristic", color='r')
